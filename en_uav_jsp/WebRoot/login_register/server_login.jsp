...
<%
	...
	System.out.println("获得的参数是：username="+username+",ps="+ps);
	//开始查询数据库
	...
	try {
		Connection conn = DriverManager.getConnection("jdbc:mysql://localhost:3306/test?user=root&password=admin&useUnicode=true&characterEncoding=UTF-8");
	...
	json.put("result_msg","error");	//该用户名没有被注册
	json.put("result_code",-1);	//返回0表示正常，不等于0就表示有错误产生，错误代码
	}
	else{
	json.put("result_msg","ok"); //该用户名已经被注册
